...
### 📊 GTM / GA4 導入・プライバシーポリシー・画像最適化（✅ 2026-08-10 本番公開済み）

**GTM 導入**

- コンテナ ID: `GTM-KWVH8WFT`。[vite.config.ts](vite.config.ts) の `gtmPlugin()` が
  **本番ビルド時だけ**全ページの `<head>` 先頭にスニペットを注入する
- **判断: HTML 4枚（→5枚）に直書きせずプラグインにした。** ページを増やすたびに
  入れ忘れる事故を防ぐため。`apply: 'build'` なので `npm run dev` には入らず、
  自分の開発アクセスは計測されない
- **判断: GA4 のスニペットはサイトに埋めない。** GA4 設定タグは GTM 管理画面側で作る。
  両方埋めると全ページが二重カウントされる
- ID が空・不正ならビルドを通したまま埋め込みをスキップ（壊れたタグを公開しないため）。
  当初 `GTM-XXXXXXX` のプレースホルダが正規表現 `^GTM-[A-Z0-9]+$` を通ってしまうバグがあり修正
- **GTM 管理画面側の設定はユーザー作業（未実施）**: GA4 設定タグ作成 → トリガー All Pages → 公開

**プライバシーポリシー**

- `/privacy/` を新設（[privacy/index.html](privacy/index.html) + [src/privacy/main.ts](src/privacy/main.ts)）。
  vite.config の `rollupOptions.input` に `privacy` を登録
- **理由: Google アナリティクス利用規約が Cookie 利用の開示を求めている。** GA を入れるなら必須
- 内容: アクセス解析と Cookie / オプトアウト方法（GA オプトアウトアドオン）/
  カードのコレクションが localStorage に保存され外部送信しないこと / 外部リンク / 連絡先
- 導線として [src/shared/footer.ts](src/shared/footer.ts) を新設し、全5ページに設置。
  **プライバシーポリシーへのリンクはフッターが唯一なので、ページを増やすときは必ず `createFooter()` を呼ぶこと**
- nav.ts の実行時 `<style>` 注入は真似せず、footer.css を import する形にした

**画像最適化**

- **public/images 配下の png/jpg 53枚をすべて WebP に置換（3601KB → 1299KB、64%削減）**。
  透過を保つ必要があるキャラ画像が多く、PNG のままでは削れないため。品質は png=85 / jpg=80
- **`games/runner/sugimoto.png` と `chars/sugimoto.png` がバイト単位で同一だった**（前川も同様）。
  複製を消してランナーからも `chars/` を参照するようにし、キャッシュが効くようにした
- `<img>` に `width`/`height` を付与（CLS 対策）。`reading.webp`・ゲーム一覧サムネ・
  カード画像に `loading="lazy"`
- **カードのコレクション20種（旧2MB）は `loading="lazy"` で初回に落とさなくなった**。
  `display:none` の中の lazy 画像はタブを開くまで読まれないため、狙いどおり効いている
- ランナーの過去回18枚の先読みを、ホスト2枚のロード完了**後**に回すよう変更。
  ホスト画像が届くまで「ホストを選ぼう」ボタンが無反応なので、そこを最優先にした
- 検証: preview サーバーに対しヘッドレスで全5ページを開き、
  **404・JSエラー・表示できない画像ゼロ**、フッターと GTM の存在を確認（puppeteer-core は検証後アンインストール）
- フッター注記と「制定日」が `--text-dim`（#ecd8f5）で薄すぎて読めなかったため `--text-secondary` に変更

**本番確認（デプロイ run 31381081393 成功後）**

- 全5ページで gtm.js?id=GTM-KWVH8WFT の読み込みと dataLayer 初期化を確認
- 404・JSエラー・表示できない画像ゼロ、フッターとプライバシーリンクも全ページに存在

**⚠️ 公開後に判明したレイアウト崩れ（同日中に修正・再公開）**

- **原因: 今回付けた `width`/`height` 属性。** 属性は CSS より優先度が低いが、
  **CSS が `height` しか指定していない画像では属性側の `width` が生き残る**ため潰れた。
  該当: ゲーム一覧のキャラ（`.header-char`）、ナビロゴ、ヒーローのロゴとキャラ、カードのパック
- 修正: base.css に `:where(img[width][height]) { width: auto; height: auto; }` を追加。
  **`:where()` で詳細度 0 にしてあるので個別 CSS は普通に上書きでき、
  アスペクト比は属性から効き続けるので CLS 対策は維持される**
- 検証: 全ページを PC(1280) / スマホ(390) 幅で開き、
  **各 `<img>` の表示比率と本来の比率のズレを自動チェック**（`object-fit: cover` の1件を除きゼロ）
- 教訓: `width`/`height` 属性を足すときは、**片方だけ指定している CSS が無いか必ず確認する**

**次にやること**

- **GTM 管理画面で GA4 設定タグを作って公開**（サイト側は完了。これをやるまで GA4 にデータは入らない）
- コードレビューで挙がった残りの高優先度（スマホのランナー: `pointercancel` 未処理・
  障害物の画面内スポーン・空中障害物の低すぎる高度、カードの「めくる前に保存」）
- OGP メタタグ（本文が JS 生成なので静的 HTML の `<head>` に直書きが必須）
